    ...
    def preprocess(example):
        ...
        if augment_name is not None and 'mixup' in augment_name:
            ...
            if 'mixup_' in augment_name:
                ...
                if any(alpha) and re.match(r'^-?\d+(?:\.\d+)?$',
                                           alpha[0]) is not None:
...
def my_mixup(batch):
    ...
    images = (ratio * batch['images'][:bs] +
              (1.0 - ratio) * batch['images'][bs:])
    mix_labels = batch['labels'][bs:]
    labels = batch['labels'][:bs]
...
def _preprocess_image(image_bytes: tf.Tensor,
                      ...
                      eval_preproc: str = 'crop_resize') -> tf.Tensor:
    ...
    if is_training:
        ...
        if eval_preproc == 'crop_resize':
            ...
            image = _decode_and_resize_then_crop(image_bytes,
...
def _decode_and_random_crop(
        ...
        jpeg_shape: Optional[tf.Tensor] = None) -> tf.Tensor:
    """Make a random crop of chosen size."""
    if jpeg_shape is None:
        jpeg_shape = get_shape(image_bytes)
    bbox = tf.constant([0.0, 0.0, 1.0, 1.0], dtype=tf.float32, shape=[1, 1, 4])
    image = _distorted_bounding_box_crop(image_bytes,
