...
class RandAugment(object):
    ...
    """
        ...
        self.prob_to_apply = (float(prob_to_apply)
                              if prob_to_apply is not None else None)
    ...
    def _get_level(self, rng):
        ...
        if self.magstd:
            ...
            assert self.num_levels, (
                'When magstd is provided, num_levels has to be '
                'given.')
            assert self.num_levels >= self.level
            level = tf.random.stateless_normal(shape=[],
                                               seed=rng,
                                               mean=self.level,
                                               stddev=self.magstd,
                                               dtype=tf.float32)
        ...
        if self.num_levels is None:
            ...
            level = tf.random.stateless_uniform(shape=[],
                                                seed=rng,
    ...
    def _apply_one_layer(self, rng, image):
        ...
        branch_index = tf.random.stateless_uniform(shape=[],
                                                   ...
                                                   dtype=tf.int32)
        aug_image = tf.switch_case(branch_index,
                                   branch_fns,
                                   default=lambda: image)
        if self.prob_to_apply is not None:
            return tf.cond(
                tf.random.stateless_uniform(
                    shape=[], seed=rng_c, dtype=tf.float32) <
