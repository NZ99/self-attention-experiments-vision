...
from data.constants import DEFAULT_CROP
...
def color_jitter_nonrand(image,
  ...
  """
    with tf.name_scope('distort_color'):
        ...
        for i in range(4):
            image = apply_transform(i, image, brightness, contrast, saturation,
                                    hue)
...
def _compute_crop_shape(image_height, image_width, aspect_ratio,
  ...
  """
    ...
    def _requested_aspect_ratio_wider_than_image():
        crop_height = tf.cast(
            tf.rint(crop_proportion / aspect_ratio * image_width_float),
            tf.int32)
        crop_width = tf.cast(tf.rint(crop_proportion * image_width_float),
                             tf.int32)
    ...
    def _image_wider_than_requested_aspect_ratio():
        crop_height = tf.cast(tf.rint(crop_proportion * image_height_float),
                              tf.int32)
        crop_width = tf.cast(
            tf.rint(crop_proportion * aspect_ratio * image_height_float),
            tf.int32)
        return crop_height, crop_width
...
def preprocess_for_eval(image, height, width, crop=True):
  ...
  """
    if crop:
        image = center_crop(image, height, width, crop_proportion=DEFAULT_CROP)
    image = tf.reshape(image, [height, width, 3])
    image = tf.clip_by_value(image, 0., 1.)
    return image
